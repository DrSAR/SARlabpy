...
data_dict_template['data'] = numpy.ones([100,50,6,110])

filler = numpy.arange

data_dict_template['data'][50:78,22:42,0:6,10] = 11
data_dict_template['data'][50:78,22:42,0:6,10:110] = numpy.arange(11,1,-0.1)
data_dict_template['header']['method']['PVM_RepetitionTime'] = 1000/50
data_dict_template['header']['method']['PVM_NRepetitions'] = 110

norm_data = sar.normalize_dce(data_dict_template)
inj_point = sar.inj_point(data_dict_template)
auc_template = sar.calculate_AUC(data_dict_template)
...
base = data_dict_template['header']['method']['PVM_NRepetitions'] - (inj_point)
height = numpy.round(numpy.max(norm_data[:,:,:,:]),2)

area_triangle = base*height/2


print "The area of the triangle should be %f and it is actually " % area_triangle + str(numpy.max(auc_template))
